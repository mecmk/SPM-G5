...
Added by story 5.1 (assign a coordinator), which is the first story that needs to read and
write an event row. The mapping covers the whole ``events`` table so later stories in the 2.x
/ 3.x / 4.x / 6.x / 7.x epics **extend this class** rather than mapping ``events`` a second
time - two declarative classes on one table raise ``InvalidRequestError``.
"""
