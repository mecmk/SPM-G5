...
| `e2e/bookings.spec.ts` | 13.1 venue staff booking requests queue: pending entries with their summary, the detail page, several requests across events/venues/dates, decided requests excluded, no coordinator entry point; 13.2 approve from the queue card or detail page; 13.2.1 reject with a mandatory reason: the dialog names the booking, validates the reason, can be cancelled, preserves a typed reason on failure, and the requesting coordinator reaches the outcome read-only through Events inbox |
